...
public sealed class DocumentConfiguration : IEntityTypeConfiguration<Document>
{
    public void Configure(EntityTypeBuilder<Document> builder)
    {
        ...
        builder.Property(d => d.KnowledgeStatus).HasConversion<string>().HasMaxLength(50);
        builder.Property(d => d.AIEnrichmentStatus).HasConversion<string?>().HasMaxLength(50);
        builder.Property(d => d.FailureReason).HasMaxLength(500).IsRequired(false);

        builder.HasIndex(d => d.UserId);
        builder.HasIndex(d => d.ProcessingStatus);
        builder.HasIndex(d => d.KnowledgeStatus);
