    ...
    <div class="min-h-screen flex items-center justify-center bg-gray-100 p-4">
      <div class="w-full max-w-md card">
        ...
        <form [formGroup]="form" (ngSubmit)="onSubmit()">
          <div class="mb-4">
            <label for="reg-email" class="label-base">Email</label>
            <input id="reg-email" type="email" formControlName="email" class="input-base"
          ...
          <div class="mb-5">
            <div class="flex items-center justify-between mb-1.5">
              <label for="reg-password" class="label-base mb-0">Password (min 6 characters)</label>
            ...
            <input id="reg-password" [type]="showPassword() ? 'text' : 'password'" formControlName="password"
                   class="input-base" autocomplete="new-password" />
            @if (form.get('password')?.invalid && form.get('password')?.touched) {
              <p class="text-red-600 text-xs mt-1" role="alert">{{ form.get('password')?.getError('serverErrors')?.[0] ?? form.get('password')?.getError('serverMessage') ?? 'Password required (min 6 characters)' }}</p>
            }
          </div>
        ...
        <p class="mt-5 text-center text-sm text-gray-600">
          Already have an account? <a routerLink="/auth/login" class="font-medium text-blue-600 hover:text-blue-700 hover:underline">Sign in</a>
        </p>
      </div>
    </div>
  `
